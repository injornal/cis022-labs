...
class Test:
    ...
    def test(self, a, b):
        if a == b:
            ...
            self.print(f"Failed: Result - {a}, Expected - {b}")
...
def test():
    ...
    tester.test(tree.breadth_first_traversal(), bft_result)
    in_result = sorted(value_array)
    tester.test(tree.inorder_traversal(), in_result)
    pre_result = [57.12, 23.44, 18.36, 11.00, 1.00, 20.21, 21.00, 44.55, 36.18, 48.48, 51.00, 57.12, 68.99, 77.77,
                  111.22, 101.00, 543.21, 345.67, 251.00, 151.00]
    tester.test(tree.preorder_traversal(), pre_result)
    post_result = [1.00, 11.00, 20.21, 21.00, 18.36, 36.18, 51.00, 48.48, 44.55, 23.44, 77.77, 68.99, 101.00, 151.00,
                   251.00, 345.67, 543.21, 111.22, 87.43, 57.12]
    tester.test(tree.postorder_traversal(), post_result)
